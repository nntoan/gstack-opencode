...
\`\`\`bash
gstack browse js "JSON.stringify(performance.getEntriesByType('navigation')[0])"
\`\`\`
...
Resource analysis:
\`\`\`bash
gstack browse js "JSON.stringify(performance.getEntriesByType('resource').map(r => ({name: r.name.split('/').pop().split('?')[0], type: r.initiatorType, size: r.transferSize, duration: Math.round(r.duration)})).sort((a,b) => b.duration - a.duration).slice(0,15))"
\`\`\`

Bundle size check:
\`\`\`bash
gstack browse js "JSON.stringify(performance.getEntriesByType('resource').filter(r => r.initiatorType === 'script').map(r => ({name: r.name.split('/').pop().split('?')[0], size: r.transferSize})))"
gstack browse js "JSON.stringify(performance.getEntriesByType('resource').filter(r => r.initiatorType === 'css').map(r => ({name: r.name.split('/').pop().split('?')[0], size: r.transferSize})))"
\`\`\`

Network summary:
\`\`\`bash
gstack browse js "(() => { const r = performance.getEntriesByType('resource'); return JSON.stringify({total_requests: r.length, total_transfer: r.reduce((s,e) => s + (e.transferSize||0), 0), by_type: Object.entries(r.reduce((a,e) => { a[e.initiatorType] = (a[e.initiatorType]||0) + 1; return a; }, {})).sort((a,b) => b[1]-a[1])})})()"
\`\`\`
...
export const benchmarkSkill: GstackSkill = {
  name: 'benchmark',
  description:
    'Performance regression detection — baseline page load times, Core Web Vitals, and resource sizes. Compare before/after on every PR to catch bundle size regressions.',
  template: transformSkillContent(rawTemplate),
  group: 'browser',
  originalSkillName: 'gstack-benchmark',
