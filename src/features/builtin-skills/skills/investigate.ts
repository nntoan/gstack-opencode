...
const rawTemplate = `> **Safety Advisory:** This skill includes safety checks that verify file edits are within the allowed scope boundary before applying, and verify file writes are within the allowed scope boundary before applying. When using this skill, always pause and verify before executing potentially destructive operations. If uncertain about a command's safety, ask the user for confirmation before proceeding.
...
- **When estimating effort**, always show both scales: human team time and AI+gstack time. The compression ratio varies by task type — use this reference:
...
Log eureka moments:
\`\`\`bash
jq -n --arg ts "\$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "investigate" --arg branch "\$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:\$ts,skill:\$skill,branch:\$branch,insight:\$insight}' >> .gstack/analytics/eureka.jsonl 2>/dev/null || true
\`\`\`
Replace ONE_LINE_SUMMARY. Runs inline — don't stop the workflow.
...
**If the freeze skill is available:** Identify the narrowest directory containing the affected files. Write it to the freeze state file:

\`\`\`bash
STATE_DIR="\${GSTACK_DATA:-.gstack}"
mkdir -p "\$STATE_DIR"
echo "<detected-directory>/" > "\$STATE_DIR/freeze-dir.txt"
echo "Debug scope locked to: <detected-directory>/"
\`\`\`

Substitute \`<detected-directory>\` with the actual directory path (e.g., \`src/auth/\`). Tell the user: "Edits restricted to \`<dir>/\` for this debug session. This prevents changes to unrelated code. Run the unfreeze skill to remove the restriction."
...
export const investigateSkill: GstackSkill = {
  name: 'investigate',
  description:
  ...
  group: 'utility',
  originalSkillName: 'gstack-investigate',
