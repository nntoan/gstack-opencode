...
Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using AI as a force multiplier.
...
# 8. gstack analytics history (if available)
...
**Backlog Health (if TODOS.md exists):** Read \`TODOS.md\`. Compute:
...
**Skill Usage (if analytics exist):** Read \`.gstack/analytics/skill-usage.jsonl\` if it exists. Filter entries within the retro time window by \`ts\` field. Aggregate by skill name. Present as:

\`\`\`
| Skill Usage | /ship(12) /qa(8) /review(5) |
\`\`\`
...
**AI collaboration note:** If many commits have \`Co-Authored-By\` AI trailers, note the AI-assisted commit percentage as a team metric. Frame it neutrally.
...
export const retroSkill: GstackSkill = {
  name: 'retro',
  description:
  ...
  group: 'utility',
  originalSkillName: 'gstack-retro',
