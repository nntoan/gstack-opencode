...
4. **List existing design docs for this project:**
   \`\`\`bash
   ls -t .gstack/design-docs/*-design-*.md 2>/dev/null
   \`\`\`
...
\`\`\`bash
grep -li "<keyword1>\\|<keyword2>\\|<keyword3>" .gstack/design-docs/*-design-*.md 2>/dev/null
\`\`\`
...
This enables cross-session discovery — multiple sessions exploring the same project will see each other's design docs in \`.gstack/design-docs/\`.
...
**Eureka check:** If Layer 3 reasoning reveals a genuine insight, name it: "EUREKA: Everyone does X because they assume [assumption]. But [evidence from our conversation] suggests that's wrong here. This means [implication]." Log the eureka moment:
\`\`\`bash
jq -n --arg ts "\$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "office-hours" --arg branch "\$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:\$ts,skill:\$skill,branch:\$branch,insight:\$insight}' >> .gstack/analytics/eureka.jsonl 2>/dev/null || true
\`\`\`
...
**Step 3: Render (browse binary required)**

Visual sketch rendering requires the browse binary. Run the setup script to enable it.

If browse is not available, skip the render step. Tell the user: "Visual sketch rendered as HTML at \$SKETCH_FILE. Open it in your browser to preview, or run the gstack setup script to enable automatic screenshots."

**Step 4: Present and iterate**

Show the sketch location to the user. Ask: "Does this feel right? Want to iterate on the layout?"
...
\`\`\`bash
PRIOR=\$(ls -t .gstack/design-docs/*-\$BRANCH-design-*.md 2>/dev/null | head -1)
\`\`\`
If \`\$PRIOR\` exists, the new doc gets a \`Supersedes:\` field referencing it.

Write to \`.gstack/design-docs/{\$USER}-{\$BRANCH}-design-{\$DATETIME}.md\`:
...
The design doc at \`.gstack/design-docs/\` is automatically discoverable by downstream skills — they will read it during their pre-review context gathering.
...
export const officeHoursSkill: GstackSkill = {
  name: 'office-hours',
  description:
    'YC Office Hours partner — reframes your product idea before you write code. Runs Six Forcing Questions, Premise Challenge, and Alternatives Generation. Produces a design doc consumed by all downstream review skills. Use at the start of any feature, before writing a plan or implementation.',
  template: transformSkillContent(rawTemplate),
  group: 'planning',
  originalSkillName: 'gstack-office-hours',
