...
import type { BrowserManager, RefEntry } from './browser-manager.ts';
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (flag.takesValue) {
      ...
      if (flag.optionKey === 'depth') {
        (opts as Record<string, unknown>)[flag.optionKey] = parseInt(value, 10);
        if (isNaN(opts.depth!)) throw new Error('Usage: snapshot -d <number>');
      } else {
        (opts as Record<string, unknown>)[flag.optionKey] = value;
      }
    } else {
      (opts as Record<string, unknown>)[flag.optionKey] = true;
    }
  }
  return opts;
}
