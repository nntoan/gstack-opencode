...
import { resolveConfig, ensureBrowserStateDir, readVersionHash } from './config.ts';
...
// ─── State File ────────────────────────────────────────────────
function readState(): BrowseServerState | null {
  try {
    const data = fs.readFileSync(config.stateFile, 'utf-8');
    return JSON.parse(data) as BrowseServerState;
  } catch {
    return null;
  }
}
...
function cleanupLegacyState(): void {
  try {
    ...
    for (const file of files) {
      ...
      try {
        const data = JSON.parse(fs.readFileSync(fullPath, 'utf-8')) as { pid?: number };
...
// ─── Server Lifecycle ──────────────────────────────────────────
async function startServer(): Promise<BrowseServerState> {
  ...
  const serverCmd = useNode
    ? ['node', NODE_SERVER_SCRIPT as string]
...
async function ensureServer(): Promise<BrowseServerState> {
  const state = readState();

  if (state && isProcessAlive(state.pid)) {
    const currentVersion = readVersionHash();
    if (currentVersion && state.binaryVersion && currentVersion !== state.binaryVersion) {
      process.stderr.write('[browse] Binary updated, restarting server...\n');
  ...
  process.stderr.write('[browse] Starting server...\n');
  return startServer();
}

// ─── Command Dispatch ──────────────────────────────────────────
async function sendCommand(
  state: BrowseServerState,
  command: string,
  args: string[],
  retries = 0
): Promise<void> {
  ...
  try {
    ...
    if (resp.status === 401) {
      process.stderr.write('[browse] Auth failed — server may have restarted. Retrying...\n');
    ...
    if (resp.ok) {
      ...
      try {
        ...
        process.stderr.write((err.error || text) + '\n');
    ...
    if (e.name === 'AbortError') {
      process.stderr.write('[browse] Command timed out after 30s\n');
      process.exit(1);
    }
    if (
      e.code === 'ECONNREFUSED' ||
      ...
      process.stderr.write('[browse] Server connection lost. Restarting...\n');
...
async function main(): Promise<void> {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    process.stdout.write(`gstack browse — Fast headless browser for AI coding agents
