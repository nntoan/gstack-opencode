...
import { getCookiePickerHTML } from './cookie-picker-ui.ts';
...
function jsonResponse(data: unknown, opts: { port: number; status?: number }): Response {
...
export async function handleCookiePickerRoute(
  ...
  try {
    ...
    if (pathname === '/cookie-picker/import' && req.method === 'POST') {
      let body: { browser?: string; domains?: unknown };
      try {
        body = (await req.json()) as { browser?: string; domains?: unknown };
      } catch {
      ...
      const result = await importCookies(browser, domains as string[]);
    ...
    if (pathname === '/cookie-picker/remove' && req.method === 'POST') {
      ...
      try {
        body = (await req.json()) as { domains?: unknown };
      } catch {
        return errorResponse('Invalid JSON body', 'bad_request', { port });
      }
      ...
      log(`[cookie-picker] Removed cookies for ${(domains as string[]).length} domains`);

      return jsonResponse(
        {
          removed: (domains as string[]).length,
          domains,
        },
        { port }
      );
    ...
    log(`[cookie-picker] Error: ${message}`);
    return errorResponse(message || 'Internal error', 'internal_error', { port, status: 500 });
  }
}
