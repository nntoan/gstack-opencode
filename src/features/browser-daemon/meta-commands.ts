...
import { getCleanText } from './read-commands.ts';
import { READ_COMMANDS, WRITE_COMMANDS, META_COMMANDS } from './commands.ts';
...
export async function handleMetaCommand(
  ...
  switch (command) {
    ...
    case 'restart': {
      log('[browse] Restart requested. Exiting for CLI to restart.');
      await shutdown();
      return 'Restarting...';
    }
    ...
    case 'responsive': {
      ...
      for (const vp of viewports) {
        ...
        const vpPath = `${prefix}-${vp.name}.png`;
        await page.screenshot({ path: vpPath, fullPage: true });
        results.push(`${vp.name} (${vp.width}x${vp.height}): ${vpPath}`);
      }
