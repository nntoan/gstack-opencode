import type { WebsearchMcpOverride } from '../types/config.ts';
import type { RemoteMcpConfig } from './types.ts';

export function createWebsearchConfig(config?: WebsearchMcpOverride): RemoteMcpConfig {
  const provider = config?.provider ?? 'exa';

  if (provider === 'tavily') {
    const tavilyKey = process.env.TAVILY_API_KEY ?? config?.api_key;
    return {
